...
const TournamentFeed: React.FC = () => {
  ...
  const splitInfo = (text: string) => {
    if (!text) return { latest: "Fetching latest results...", next: "Confirming schedule..." };
    // Faster, smarter parsing for the "briefing" format
    const parts = text.split(/Next Match|Upcoming Match|Next:|\||Schedule:/i);
    return {
      latest: parts[0]?.replace(/Latest Result:|Latest Score:|Score:|Briefing:/i, '').trim() || "Result Pending",
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-20">
            ...
            <div className="bg-black/80 border-2 border-warrior-orange/50 rounded-[40px] p-10 md:p-12 shadow-[0_0_60px_rgba(179,93,40,0.15)] relative overflow-hidden transition-all hover:border-warrior-orange hover:shadow-[0_0_80px_rgba(179,93,40,0.25)] group">
              <div className="flex justify-between items-center mb-10">
                <div className="flex items-center space-x-3">
                  <span className="w-3 h-3 rounded-full bg-red-500 animate-pulse"></span>
                  <span className="text-white font-black uppercase tracking-widest text-[10px]">Live Sync</span>
                </div>
                <span className="bg-warrior-orange/20 text-warrior-orange border border-warrior-orange/30 text-[9px] font-black px-4 py-1 rounded-full uppercase tracking-widest">Latest Score</span>
              </div>
              ...
              {loading ? (
                ...
                <div className="space-y-6">
                  <h3 className="text-white font-oswald text-4xl md:text-6xl leading-[1.1] uppercase italic tracking-tight group-hover:scale-[1.02] transition-transform origin-left">
                    {latest}
                  </h3>
                  <div className="flex items-center text-slate-500 font-bold text-xs uppercase tracking-[0.2em] pt-6 border-t border-white/5">
                    <i className="fa-solid fa-location-dot mr-3 text-warrior-orange"></i>
                    USHSC Tournament Series
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-[40px] p-10 md:p-12 backdrop-blur-xl relative overflow-hidden transition-all hover:bg-white/[0.08]">
               <div className="flex justify-between items-center mb-10">
                ...
                <i className="fa-solid fa-clock-rotate-left text-white/10 text-2xl"></i>
              </div>
              ...
              {loading ? (
                ...
                <div className="space-y-8">
                  <p className="text-slate-200 text-2xl md:text-4xl font-medium italic border-l-4 border-warrior-orange pl-8 leading-tight">
                    {next}
                  </p>
                  <div className="flex items-center space-x-3">
                    <span className="bg-emerald-500/10 text-emerald-500 text-[10px] font-black px-3 py-1 rounded uppercase tracking-widest">Verified Venue</span>
                  </div>
                </div>
              )}
            </div>
